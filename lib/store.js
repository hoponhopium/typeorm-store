...
class Store {
    ...
    async upsert(e, primaryKeys) {
        if (Array.isArray(e)) {
            ...
            await this.changes?.trackUpsert(entityClass, [e]);
            await this.em().upsert(entityClass, e, primaryKeys ?? ['id']);
        }
    }
    async saveMany(entityClass, entities, primaryKeys) {
        ...
        if (fk.length == 0)
            return this.upsertMany(em, entityClass, entities, primaryKeys);
    ...
    async upsertMany(em, entityClass, entities, primaryKeys) {
        for (let b of splitIntoBatches(entities, 1000)) {
            await em.upsert(entityClass, b, primaryKeys ?? ['id']);
        }
    }
    async insert(e) {
        if (Array.isArray(e)) {
